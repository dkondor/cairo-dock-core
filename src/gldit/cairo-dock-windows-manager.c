...
{
	if (actor->bIsSticky) return TRUE;
	if (GPOINTER_TO_INT (s_backend.flags) & GLDI_WM_NO_VIEWPORT_OVERLAP)
		return ( (actor->iNumDesktop == -1 || actor->iNumDesktop == g_desktopGeometry.iCurrentDesktop) &&
			actor->iViewPortX == g_desktopGeometry.iCurrentViewportX && actor->iViewPortY == g_desktopGeometry.iCurrentViewportY);
	if (GPOINTER_TO_INT (s_backend.flags) & GLDI_WM_GEOM_REL_TO_VIEWPORT)
		return gldi_window_is_on_desktop (actor, g_desktopGeometry.iCurrentDesktop, g_desktopGeometry.iCurrentViewportX, g_desktopGeometry.iCurrentViewportY);
...
gboolean gldi_window_is_on_desktop (GldiWindowActor *pAppli, int iNumDesktop, int iNumViewportX, int iNumViewportY)
{
	...
	if (GPOINTER_TO_INT (s_backend.flags) & GLDI_WM_NO_VIEWPORT_OVERLAP)
		return ( (pAppli->iNumDesktop == -1 || pAppli->iNumDesktop == g_desktopGeometry.iCurrentDesktop) &&
			pAppli->iViewPortX == iNumViewportX && pAppli->iViewPortY == iNumViewportY);
